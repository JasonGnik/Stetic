...
**Round-3 restructure — DONE (commit 0172630):** **Funnel reordered** → `capture → bluff(fake scan) → IDENTITY transformation → signin → paywall → real scan → result`; identity moved out of onboarding into the funnel post-scan (answers threaded via `IdentityInputs`); **deleted** tease/lock + fomo from the flow. **Onboarding:** cut callback/nutrition/social-proof screens; **measurements before pace**; emotional block moved to the end. **Carousel** now **swipeable** + the climb beat shows a real score climb (7.2 Diamond → 9.0 Mythic w/ gridlines); plan beat shows real app rows. **DOOM** bottom line = "The time will pass anyways." **Mountain:** sleeping removed; **3 previewable variants** (`STETIC_MTN=1` path+hiker · `2` silhouette · `3` spotlight) — Jason to pick. Dev: `STETIC_FUNNEL_PHASE=identity`.

**Round-2 polish — DONE (commit 399b1c5):** AHA generic figure → **how-it-works auto-demo** (4 beats, mini real-app mockups: scan+score → weak-point bars → plan w/ flagged weak points → progress climb) · **mountain now ZOOMS** (wide goal → push in on hiker → quote surfaces) · **nutrition shows a meal-scan card** + short copy (no fat paragraph) · **trash = real flames + slower readable toss** · funnel projection reverted to the **with-vs-without graph** (dropped two-bodies) · **trial-reminder screen deleted** · DOOM "float around". Preview: `STETIC_ONB_STEP=10` (demo) `=11` (mountain zoom) `=12` (nutrition) `=27` (trash); `STETIC_FUNNEL_PHASE=fomo` (graph).

**Signature visuals — earlier (commit d9b2b34):** ⛰️ cinematic mountain (day→dusk→night cycle, hiker walks to camp + campfire, summit flag, "just focus on today" + Tolstoy) · 🫥 AHA "mock scan" (figure + sweeping scan line + scan brackets + lit weak point) · 🗑️ transformation Act 1 toss→burn (cards fall into bin → rising embers) · 📈 funnel projection = before→after physique (soft "now" → lean V-taper "12 weeks", replaced the Gold→Diamond tier pills). Preview: `STETIC_ONB_STEP=10` (aha) `=11` (mountain) `=27` (transformation); `STETIC_FUNNEL_PHASE=fomo` (before→after). Figures are geometric/composed-shape — open to a more refined art pass if Jason wants.
