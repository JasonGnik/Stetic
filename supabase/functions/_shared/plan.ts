...
export interface PlanProfile {
  ...
  pace?: string;              // slow | recommended | aggressive
...
- muscle_breakdown: for EACH of chest, back, shoulders, arms, legs, abs:
  ...
  • detail: 2-3 full sentences — what is developed WELL, then what is specifically LACKING or holding the group back (e.g. "lats have width but the lower lats are thin", "side delts trail the front delts and cap your shoulder width"). Be specific and visual.${profile.current_split ? " If their current split under-trains or junk-volumes this group, say so plainly here — connect the lagging group to what their routine is (or isn't) doing." : ""}
  • sub: 1-3 sub-muscles (biceps/triceps, side/rear/front delts, upper/lower abs, lats/upper-back, quads/hamstrings). For EACH sub give: name, a short status label (e.g. "Strong", "Developed", "Lagging", "Needs width", "Underdeveloped"), and a one-line training cue for bringing it up or maintaining it.
- projection: AMBITIOUS but still grounded in real training science. Two milestones at 6 and 12 weeks of following THIS plan. Gains scale by starting level: a beginner / higher-body-fat / lifter far below their potential is in their fastest growth window ("newbie gains") and can move quickly — realistically +0.5 to +1.0 over 6 weeks and +1.0 to +2.0 over 12 weeks when starting in the Bronze/Silver range. A mid-level lifter improves moderately (≈ +0.3-0.6 over 6 weeks). An advanced, lean, near-potential lifter improves slowly (+0.1-0.3 over 6 weeks). Lean into the early momentum — these people CAN change fast. NEVER exceed their potential (${scan.potential}); if close to potential, gains taper toward it. For each milestone give: weeks, projected_score (one decimal), projected_tier (lowercase: bronze/silver/gold/platinum/diamond/elite/mythic/greek_god), points_gain, projected_body_fat (realistic — drops a few % with a fat-loss goal, roughly flat on a lean-gain; current is ${scan.body_fat}%), a one-line summary, and muscle_gains for their 2-3 weakest groups as {group, from, to}. Honesty matters — the user re-scans and this gets checked.${profile.current_split ? "\n- split_critique: see instruction above." : ""}
- No medical claims.`;
}
